...
//            sb.append('0');
			...
			// trailing zeroes
			/*
			 * // AND possibly trim trailing ones int i = nanosLen; while ((i > 1) &&
			 * nanoSB.charAt(i-1) == '0') { --i; } if (i < nanosLen) { nanoSB.setLength(i);
			 * }
			 */
			sb.append(nanoSB);
		}
	...
	/**
	 * Extracts the seconds and nanoseconds component of {@code seconds} as
	 * {@code long} and {@code int} values, passing them to the given converter. The
	 * implementation avoids latency issues present on some JRE releases.
	 *
	 * @since 2.9.8
	 */
	public static <T> T extractSecondsAndNanos(BigDecimal seconds, BiFunction<Long, Long, T> convert) {
		...
		if (nanoseconds.precision() - nanoseconds.scale() <= 0) {
			...
			if (secondsOnly < 0 && secondsOnly > Instant.MIN.getEpochSecond()) {
				// Issue #69 and Issue #120: avoid sending a negative adjustment to the Instant
				// constructor, we want this as the actual nanos
